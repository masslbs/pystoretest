    ...
    for item in abi:
        if item["type"] == "error":
            ...
            error_signature = (
                item["name"]
                + "("
                + ",".join([input["type"] for input in item["inputs"]])
                + ")"
            )
            # Generate the selector
            selector = generate_selector(error_signature)
            # print(f"found error type: {item} with selector: {selector}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Match Ethereum error selector with contract ABI."
    )
    parser.add_argument(
        "abi_filename", type=str, help="The filename of the contract ABI JSON"
    )
    ...
    if error_selector.startswith("0x"):
        error_selector = error_selector[2:]
