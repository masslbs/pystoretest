...
class CoinGecko:
  ...
  def get_erc20_price(self, token):
    if token not in self.erc20_token_addresses:
      raise ValueError(f"Token {token} not found")
    token_address = self.erc20_token_addresses[token]
    url = f"https://api.coingecko.com/api/v3/simple/token_price/ethereum?contract_addresses={token_address}&vs_currencies={self.base_currency}&precision=full"
    if self.demo_key:
      url += f"&x_cg_demo_api_key={self.demo_key}"
    headers = {
      "accept": "application/json",
